...
function Page (rawData) {
  ...
  if(p.Rows.length){
    let halfLength = p.Rows.length / 2
    let half1 = p.Rows.slice(0,halfLength)
    let half2 = p.Rows.slice(halfLength, p.Rows.length)
  ...
  p.RowsContent = p.Rows.map((row) =>{
    let componentType = row[COMPONENT].split('.')[1]
    let value = {}
...
function Row(rawData){
  let p = BaseService.defaultDataConstructor(rawData)
  p.colorThemeClass = (p.ColorTheme === undefined || !p.ColorTheme)?'white':p.ColorTheme.toLowerCase()
  return p
}
